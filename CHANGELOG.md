...
- `light` 主题下的文字改用无描边字体并关闭投影：暴雪默认字体自带 (1, -1) 的黑色投影，`*Outline` 变体还额外带一圈黑色描边。两者在深色底板上都不可见，但在浅色底板上会变成深色文字周围的黑边和错位副本，看起来像模糊或重影。深色主题会原样恢复各字体对象最初的投影设置。debuff 图标上的层数文字和框体下方的持续时间文字不在浅色底板上，保留描边不变。
