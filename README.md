...
当前版本：`0.10.0-beta.1`。它预创建 5 个小队按钮和 `raid1`–`raid40` 共 40 个团队按钮，点击对应方块会直接对该固定单位施放角色当前学会的友方驱散，不改变当前目标。插件只让游戏自身的 Aura Container 显示系统过滤后的一个可驱散效果，不读取或分析受保护的 aura 数据。
