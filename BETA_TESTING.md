# SimpleDispel 0.10.0-beta.4 Party/Raid 实测
...
单人时应看到 `YOU`；小队中应显示四名队员名字。进入 Raid 后小队横条会安全隐藏，改为编号 `1`–`40` 的 8×5 团队网格，只有实际存在的 `raidN` 按钮显示。
...
1. 对应的 `YOU`/队员姓名方块应出现 debuff 图标，底部名字仍可辨认。
