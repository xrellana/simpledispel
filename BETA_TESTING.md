# SimpleDispel 0.10.0-beta.5 Party/Raid 实测
...
单人时应看到 `YOU`；小队中应显示四名队员名字。进入 Raid 后小队横条会安全隐藏，改为固定 5 列、按人数自动增减行数的团队网格，只有实际存在的 `raidN` 按钮显示；每格左侧为 debuff 图标，右侧为对应成员名字。
...
1. 10/20/25/30/40 人规模下，只显示实际存在的姓名格，外框分别使用 2/4/5/6/8 行。
2. 某个团员获得可驱散效果时，对应姓名格左侧显示图标，右侧名字仍可辨认。
...
团队格使用固定 raid index，第一行是 `raid1`–`raid5`、第二行是 `raid6`–`raid10`，依此类推；不按名字、职业、职责或 debuff 重新排序。名字只直接显示，点击目标始终由格子固定绑定的 `raidN` 决定。战斗中人数变化时，外框允许暂时保持旧高度，并应在脱战后自动更新。
...
- 点击某姓名格却驱散另一人：立即停止使用并报告，这是最高优先级单位绑定错误。
