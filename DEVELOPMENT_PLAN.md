...
- [ ] 预创建 `player`、`party1` 至 `party4`。
- [ ] 每个 unit token 永久绑定对应按钮，不根据 aura 换目标。
- [ ] 建立普通层与安全层的清晰父子关系。
- [ ] 战斗中禁止修改 protected attribute。
- [ ] 需要更新时标记 pending，并在 `PLAYER_REGEN_ENABLED` 后执行。

#### 任务 2.2：驱散技能解析

- [ ] 建立职业和专精候选技能表。
- [ ] 从 spellbook 确认角色实际学会的技能。
- [ ] 使用客户端返回的本地化技能名称设置安全按钮。
- [ ] 监听 `SPELLS_CHANGED`。
- [ ] 监听专精和天赋变化。
- [ ] 当前角色没有友方驱散时显示不可用状态。
- [ ] 无法可靠自动识别时提供脱战手动选择作为降级方案。
...
- [ ] 每个单位按钮拥有独立 Aura Container。
- [ ] 默认显示一个可驱散效果。
- [ ] 使用系统提供的驱散类型边框或符号。
- [ ] 显示持续时间或原生冷却转圈。
- [ ] 鼠标悬停显示原生 tooltip。
- [ ] 不监听 Aura Button 的显示、隐藏或尺寸变化。
- [ ] 不根据 aura 是否存在修改父按钮颜色、声音或布局。

#### 任务 2.4：基础布局

- [ ] 单人状态可选择隐藏或只显示自己。
- [ ] 小队按固定顺序显示五个方块。
- [ ] 不存在的 `partyN` 显示为空位或安全隐藏。
- [ ] `/sd unlock` 解锁拖动。
- [ ] `/sd lock` 锁定位置。
- [ ] `/sd reset` 恢复默认位置。
...
入口条件：五人版本稳定，且 40 个 Aura Container 的性能可接受。

- [ ] 预创建 `raid1` 至 `raid40` 安全按钮。
- [ ] 小队和团队使用不同的固定按钮集合。
- [ ] 只在安全时切换整体布局。
- [ ] 支持每行数量设置。
- [ ] 支持横向或纵向增长。
...
- [ ] `SimpleDispelDB` SavedVariables，包含 schema version。
- [ ] 方块大小。
- [ ] 整体缩放。
- [ ] 间距和每行数量。
- [ ] 横向、纵向增长。
...
- [ ] 锁定、解锁、重置。
- [ ] 不涉及真实 aura 的布局测试模式。
- [ ] `/sd status` 输出版本、当前技能和 API 能力检测结果。
